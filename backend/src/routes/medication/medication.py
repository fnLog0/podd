...
@router.post("/log", response_model=MedicationLogResponse, status_code=status.HTTP_201_CREATED)
async def log_medication(
    ...
    from src.schemas.medication import MedicationLogEventDefinition
    log_id = locusgraph_service.new_id()
    context_id = MedicationLogEventDefinition.get_context_id(log_id)
    taken_at = log_data.taken_at or datetime.now(timezone.utc)

    # Look up medication_id if only medication_name is provided
    medication_id = log_data.medication_id
    if not medication_id and log_data.medication_name:
        memories = await locusgraph_service.retrieve_context(
            query=f"medication {log_data.medication_name} user {current_user.id}",
            limit=10,
        )
        for memory in memories:
            payload = memory.get("payload", {})
            if (
                ...
                and payload.get("name") == log_data.medication_name
            ):
        ...
        if not medication_id:
            raise HTTPException(
                ...
                detail=f"Medication '{log_data.medication_name}' not found. Please create the medication first.",
            )
    ...
    # Use related_to from query parameter or from request body
    related_to_list = None
    if related_to:
        related_to_list = [related_to]
    elif log_data.related_to:
        related_to_list = (
            [log_data.related_to]
            if isinstance(log_data.related_to, str)
            else log_data.related_to
        )
