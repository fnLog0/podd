...
def mock_locusgraph_service():
    ...
    async def mock_store_event(*args, **kwargs):
        nonlocal id_counter
        id_counter += 1
        event_id = kwargs.get("context_id") or (args[1].get("context_id") if len(args) > 1 else None)
    ...
    with patch("src.routes.food_log.locusgraph_service", mock_service), \
         patch("src.routes.vitals.locusgraph_service", mock_service), \
         patch("src.routes.medication.locusgraph_service", mock_service), \
         patch("src.routes.medication_schedule.locusgraph_service", mock_service), \
         patch("src.routes.profile.locusgraph_service", mock_service), \
         patch("src.routes.health.locusgraph_service", mock_service):
        yield mock_service
